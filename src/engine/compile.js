  ...
  //   `x not starts with y` (with preceding `not` token already handled by caller)
  {
    ...
    for (let i = 0; i < tokens.length; i++) {
      ...
      if (t.value === 'starts' || t.value === 'ends' || t.value === 'matches') {
        // If there's already a top-level `is`, let transformIsTest handle it
        const hasTopLevelIs = tokens.some(tok => tok.type === 'identifier' && tok.value === 'is');
        if (hasTopLevelIs) continue;
        ...
        // rewrite as `subject is <test> rest` and let transformIsTest handle it
